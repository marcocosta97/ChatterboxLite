...
static int readMessage(int connfd, message_hdr_t *hdr)
{
	...
	if (msgcur >= msglen)
	{
		...
		for (size_t i = msgcur; i < msglen; ++i)
			MSGS[i].data.buf = NULL;
	}
	return 1;
}
...
static int downloadFile(int connfd, char *filename, char *sender)
{
	...
	for (;;)
	{
	     ...
	     */
			if (readMessage(connfd, &msg.hdr) <= 0)
				return -1;
		}
		break;
		default:
...
static int manage_requestreply(int connfd, operation_t *o)
{
	char *sname = o->sname;
	...
	while (!ackok)
	{
		...
		{
			if (msg.data.buf)
				fprintf(stderr, "Operazione %d FALLITA: %s\n", op, msg.data.buf);
			else
				fprintf(stderr, "Operazione %d FALLITA\n", op);
	...
	case CONNECT_OP:
	case USRLIST_OP:
	{ // ... ricevere la lista degli utenti
		if (readData(connfd, &msg.data) <= 0)
		{
			perror("reply data");
			return -1;
	...
	break;
	case GETPREVMSGS_OP:
	{ // ... ricevere la lista dei vecchi messaggi
		if (readData(connfd, &msg.data) <= 0)
		{
			perror("reply data");
			return -1;
...
// gestisce operazioni di tipo richiesta-risposta
static int manage_receive(int connfd, operation_t *o)
{
	char *sname = o->sname;
	size_t m = (size_t)-1;
...
int main(int argc, char *argv[])
{
	...
	{
		if (ops[i].op != OP_END)
			r = manage_requestreply(connfd, &ops[i]);
		else
			r = manage_receive(connfd, &ops[i]);
		if (r == 0)
			printf("Operazione %d eseguita con successo!\n", i);
		else
			break; // non appena una operazione fallisce esco
